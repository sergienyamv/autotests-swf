...
public final class Logger {
    ...
    public static synchronized Logger getInstance() {
        if (instance == null) {
            instance = new Logger();
			//System.setErr(createLoggingProxy(System.err)); //TODO: errors to log file
        }
        return instance;
    }
	...
	public void logTestEnd(final String testName, final String testStatus) {
		if (logSteps) {
			String formattedEnd = String.format("***** %1$s: '%2$s' %3$s! *****", getLoc("loc.logger.test.case"), testName, testStatus);
			String stars = "";
			int nChars = formattedEnd.length();
			for (int i = 0; i < nChars; i++) {
				stars += "*";
