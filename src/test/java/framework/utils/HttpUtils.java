...
public class HttpUtils {
        ...
        for (int i = 0; i <= 2; i++) {
            try {
                ...
                if (result < ERROR_CODE) {
                    logger.info("Success: " + logName);
                    logger.info("Title: " + StringFunctions.regexGetMatchGroup(response, "<title>([\\s\\S]*)</title>", 1));
                } else {
                    logger.info(result + "");
                    logger.warn("Failed: " + logName);
                }
				return httpClient;
			} catch (Exception e) {
				logger.debug(this, e);
    ...
    public String extractStringViaRegExpFromResponse(String regexp, int group) {
        return StringFunctions.regexGetMatchGroup(response, regexp, group);
    }
    ...
    private void logResult(int responseCode, boolean isSilent) {
        if (responseCode < ERROR_CODE) {
            if (!isSilent) {
				logger.info("Success: " + responseCode);
                logger.info("Title: " + StringFunctions.regexGetMatchGroup(response, "<title>([\\s\\S]*)</title>", 1));
            }
        } else {
            if (!isSilent) {
				logger.info("Code response:" + responseCode);
				logger.warn("Failed: " + isSilent);
            }
        }
    }
    ...
    public Set<org.openqa.selenium.Cookie> getCookies() {
		...
		return new HashSet<org.openqa.selenium.Cookie>(seleniumCookies);
	}
